...
def main():
    ...
    # print(len(m_encoded))
    while (len(m_encoded) < 125): # bytes - 3 bytes for format
        # print(m_bin.bit_length())
        m_encoded += b'0'

    m_formated = m_encoded + i2os(9, 3) # append len of message as 3 last bytes
    print(m_formated)

    Gr = mgf1(r, 128)       # oracle G
    X = xor(Gr, m_formated)
    HX = mgf1(X, k0)        # oracle H
    Y = xor(r, HX)
    ...
    # print(Y)
    # print(r)
    r = xor(Y, HX)          # recovering r
    # print(r)
    res = xor(X, Gr)
    print(res)      # formated bytes

    res_len = os2i(m_formated[125:128])
    print("Message length: {}".format(res_len))  # message len
